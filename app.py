...
def get_data():
    """구글 시트 데이터 읽기 (즉시 반영을 위해 캐시 끄기 ttl=0)"""
    try:
        # [핵심 수정 1] ttl=0으로 설정하여 등록 직후에도 즉시 새 데이터를 가져오도록 함
        df = conn.read(ttl=0)
        ...
        # [핵심 수정 2] 전화번호가 1234.0 처럼 읽히는 현상 방지
...
def check_date_reset():
    ...
    for idx, row in df.iterrows():
        ...
        if last_update != today_str:
            ...
            # 월 누적 시간에 전날 공부 시간 합산
            new_monthly = m_sec + d_sec
            
            # 달이 바뀌었으면 monthly도 초기화
...
if mode == "📺 대시보드 모드 (모니터용)":
    ...
    if not df.empty:
        ...
        for idx, row in df.iterrows():
            ...
            # 현재 공부 중인 학생은 실시간으로 시간 더해서 보여주기
...
# === 출석체크 모드 ===
    ...
    with c2:
        ...
        # secrets에 admin_password가 없으면 기본값으로 1234 사용 가능하게 처리 (테스트용)
        admin_pw = st.text_input("관리자 비밀번호 입력", type="password")
        
        # secrets가 설정되어 있으면 그것을 따르고, 없으면 테스트 통과
        is_admin = False
        if "admin_password" in st.secrets:
            if admin_pw == st.secrets["admin_password"]:
                is_admin = True
        elif admin_pw: # secrets 설정 안 했을 때 아무거나 입력하면 넘어가게 임시 조치 (필요시 수정)
             is_admin = True
